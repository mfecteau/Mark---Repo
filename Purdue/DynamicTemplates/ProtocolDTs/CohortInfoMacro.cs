...
{
	internal sealed class CohortInfoMacro
	{
		private static readonly string header_ = @"$Header: CohortInfoMacro.cs, 1, 18-Aug-09 12:03:23, Pinal Patel$";
	}
}
...
namespace TspdCfg.Purdue.DynTmplts
{
	...
	/// </summary>
	public class CohortInfoMacro : AbstractMacroImpl
	{
		
		SOA soa_ = null;
		bool DISPLAY_OTHER = false;

		public CohortInfoMacro(MacroExecutor.MacroParameters mp) : base (mp)
		{
			//
		...
		#region CohortInfoMacro


		public static MacroExecutor.MacroRetCd CohortInfo (	MacroExecutor.MacroParameters mp) 
		{
#if false
<ChooserEntry elementPath="TspdCfg.Purdue.DynTmplts.ScheduleOfProceduresMacro.ScheduleOfProcedures,ProtocolDTs.dll" elementLabel="Schedule of Procedures" ftElementType="Macro" ftMacroType="CSHARP" protected="true" editorClass="PDG.Schedule" autogenerates="true" toolTip="Prints table of Schedule of Procedures for a given Schedule of Assessments dosing days." shouldRun="true">
	<Complex>
		<ChooserEntry ftElementType="Collection" assocClass="Tspd.Icp.SOA,IcpMgr" elementPath="dummy" elementLabel="dummy" assocChooserPath="/FTICP/StudySchedule/Schedules/Schedule"/>
			...
			try 
			{
				mp.pba_.setOperation("Calculating Cohort Information", "Generating information...");
				
				CohortInfoMacro macro = null;
				macro = new CohortInfoMacro(mp);
				macro.preProcess();
				macro.display(); 
				macro.postProcess();
			...
			{
				Log.exception(e, "Error in Cohort Information Macro"); 
				mp.inoutRng_.Text = "Cohort Information Macro: " + e.Message;
			}
...
//			}
		...
		public override void display()
		{
			...
			//Creating Table 
			Word.Table table =  createTable(ref wrkRng, 14, 2);
			...
			row = table.Rows[2];
			cell = row.Cells[1];
			cell.Range.Text = "Number of Study Arm:";
			...
			cell.Range.Text = count.ToString();


			row = table.Rows[3];
			cell = row.Cells[1];
			cell.Range.Text = "Number of subjects for study arm:";
			...
			else
			{
				...
				cell.Range.Text = a.getPlannedEnrollmentPerArm();
			}


			row = table.Rows[4];
			cell = row.Cells[1];
			cell.Range.Text = "Special population required? ";
			
			cell =row.Cells[2];
			cell.Range.Text =bom_.getUserDisplayValue("/FTICP/CustomVars/SpecialPopRequired");
			

			row = table.Rows[5];
			cell1 = row.Cells[1];
			cell1.Range.Text = "DURATION ";
			...
			PeriodEnumerator pe = soa_.getPeriodEnumerator();
			//get visits in treatment period  -1
			Period wo = null;
			int incount = 0;
			int outcount = 0;
			int precount = 0;
			...
			while(pe.MoveNext())
			{
				Period current = pe.Current as Period;
				
				periodCount +=1;

				string epoch = current.getScheduleItemType();
				if(epoch.Equals("treatment"))
				{
					incount += (soa_.getProtocolEventCount(current) -1);
					precount += 1;
				}
				else if(epoch.Equals("screening"))
				{
					outcount += soa_.getProtocolEventCount(current);
					precount += soa_.getProtocolEventCount(current);
				}
				else if(epoch.ToLower().Equals("washout"))
				{
					wo = current;
				}
				else if(epoch.Equals("followUp"))
				{
					outcount += soa_.getProtocolEventCount(current);
				}
			}
			...
			row = table.Rows[6];
			cell = row.Cells[1];
			cell.Range.Text = "Number of periods in study arm(s): ";
			
			cell =row.Cells[2];
			cell.Range.Text = periodCount.ToString();
			...
			row = table.Rows[7];
			cell = row.Cells[1];
			cell.Range.Text = "Number of pre-dose period (ex. day-1, day 0) ";
			
			cell =row.Cells[2];
			cell.Range.Text =precount.ToString();


			row = table.Rows[8];
			cell = row.Cells[1];
			cell.Range.Text = "Number of days/nights per period - inpatient:  ";
			
			cell =row.Cells[2];
			cell.Range.Text =incount.ToString();

			row = table.Rows[9];
			cell = row.Cells[1];
			cell.Range.Text = "Number of days per period - outpatient visits: ";
			
			cell =row.Cells[2];
			cell.Range.Text =outcount.ToString();

			row = table.Rows[10];
			cell = row.Cells[1];
			cell.Range.Text = "Number of days between periods (washout between doses]: ";
			...
			if(wo != null)
			{				
				cell.Range.Text = (wo.getDuration() + " " + wo.getDurationTimeUnit());
			}
			else
			{
				cell.Range.Text = ("N/A");
			...
			row = table.Rows[12];
			cell = row.Cells[1];
			cell.Range.Text = "Unusual speciment handling requirements: ";
			
			cell =row.Cells[2];
			cell.Range.Text =bom_.getUserDisplayValue("FTICP/CustomVars/UnusualSpecHandReq");

			row = table.Rows[13];
			cell = row.Cells[1];
			cell.Range.Text = "Unusual data management requirements: ";
			
			cell =row.Cells[2];
			cell.Range.Text = bom_.getUserDisplayValue("FTICP/CustomVars/UnusualDataHandReq");

			row = table.Rows[14];
			cell = row.Cells[1];
			cell.Range.Text = "Any other special requirements and comments:  ";
			
			cell =row.Cells[2];
			cell.Range.Text =bom_.getUserDisplayValue("/FTICP/CustomVars/OtherSpecialReqs");
		...
		public void DisplayOtherCost()
		{
			...
			cell2 =row.Cells[2];
			cell2.Range.Text=getMedCosts() + " [cost of single study arm]";
