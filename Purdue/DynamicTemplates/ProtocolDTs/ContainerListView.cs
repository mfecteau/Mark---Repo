...
namespace TspdCfg.Purdue.DynTmplts
{
    ...
    [DesignTimeVisible(false), TypeConverter("WindowsApplication1.ListViewItemConverter")]
    ...
    [DesignTimeVisible(false), TypeConverter("WindowsApplication1.SubListViewItemConverter")]
    ...
    [DesignTimeVisible(false), TypeConverter("WindowsApplication1.ToggleColumnHeaderConverter")]
    ...
    public class ColumnHeaderCollection : CollectionBase
    {
        ...
        public virtual int Add(TspdCfg.Purdue.DynTmplts.ToggleColumnHeader colhead)
        {
        ...
        public virtual void AddRange(TspdCfg.Purdue.DynTmplts.ToggleColumnHeader[] items)
        {
            lock (List.SyncRoot)
            {
                for (int i = 0; i < items.Length; i++)
    ...
    public class ContainerListView : Control
    {
        ...
        protected TspdCfg.Purdue.DynTmplts.ColumnHeaderCollection columns;
        ...
        protected TspdCfg.Purdue.DynTmplts.ContainerListViewItem focusedItem;
        ...
        public TspdCfg.Purdue.DynTmplts.ColumnHeaderCollection Columns
        {
            get { return columns; }
        }
        ...
        public bool VisualStyles
        {
            get
            {
                ...
                try
                {
                    val = visualStyles && uxTheme.Wrapper.IsAppThemed();
                }
                catch
                {
                    val = visualStyles;
            ...
            if (headerStyle != ColumnHeaderStyle.None)
            {
                ...
                try
                {
                    ...
                    for (i = 0; i < columns.Count; i++)
                    {
                        colwid = (columns[i].ScaleStyle == ColumnScaleStyle.Spring ? springWid : columns[i].Width);
                        if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
                            uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "PRESSED", hdc,
                                lp_scr + last, tp,
                                colwid, headerBuffer,
                                lp, tp, r.Width - 6, headerBuffer);
                        else if (headerStyle != ColumnHeaderStyle.None && columns[i].Hovered)
                            uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "HOT", hdc,
                                lp_scr + last, tp,
                                colwid, headerBuffer,
                                lp, tp, r.Width - 6, headerBuffer);
                        else
                            uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc,
                                lp_scr + last, tp,
                                colwid, headerBuffer,
                                lp, tp, r.Width - 6, headerBuffer);
                        last += colwid;
                    ...
                    {
                        uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc, lp_scr + last, tp, r.Width - last - 2 + hscrollBar.Value, headerBuffer, r.Left, r.Top, r.Width, headerBuffer);
                    }
                }
                catch
                {
